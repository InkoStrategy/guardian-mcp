...
const CONTROL_TOKENS = ['0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48', '0xdAC17F958D2ee523a2206206994597C13D831ec7', '0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2', '0x6B175474E89094C44Da98b954EedeAC495271d0F', '0x2260FAC5E5542a773Aa44fBCfeDf7C193bc2C599'];
...
async function collectAttacks(drainers, want, drainerSet) {
  ...
  for (const drainer of drainers) {
    ...
    for (const h of hashes) {
      if (cases.length >= want || perDrainer >= 2 || seenTx.has(h.hash)) continue;
      let tx;
...
async function collectControls(want) {
  ...
  while (cases.length < want && from > latest - 20000) {
    ...
    let logs = [];
    try {
      logs = await provider.getLogs({ address: CONTROL_TOKENS, fromBlock: from, toBlock: to, topics: [topicApproval, null, spenders.map((s) => zeroPadValue(s, 32))] });
    } catch (err) {
...
async function replayRulesOnly(c) {
  ...
  const r = await analyze({ to: c.to, data: c.data, chainId: 1, value: c.value || '0', context: { share_threat_intel: false } }, { store: createMemoryStore(), env: {} });
  return { verdict: r.verdict, reasons: r.reasons };
}

async function replayDeployed(c) {
  const res = await fetch(BASE + '/analyze', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ to: c.to, data: c.data, chainId: 1, value: c.value || '0', context: { share_threat_intel: false } }) });
...
(async () => {
  ...
  const md = [];
  md.push('# Benchmark: real drain transactions vs. real legitimate approvals', '');
  ...
  md.push('', '## Method and limits', '', '- Attack cases are the victim-signed transactions that delivered the funds (transfers, native sends, calls to drainer contracts), which is exactly the moment Guardian is meant to intervene. Drainer-signed transferFrom pulls are excluded because the victim never sees them.', '- The ScamSniffer list is also the seed of the shared registry, so the "as deployed" column includes registry hits by construction; the "rules only" column shows what the deterministic rules catch without any list.', '- Sampling is by list order (newest entries first) and by whatever Blockscout returns; it is not a random sample of all drains and says nothing about drains that used permits, Permit2 or signatures.', '- Replays carry no `from` (simulation would run against the current state, not the historical one) and no `session_id` (the session stop signal would otherwise cascade across unrelated cases); the numbers measure transaction rules and registries only.', '- Reproduce: `node scripts/benchmark.js`. Raw data: `docs/benchmark.json`.');
