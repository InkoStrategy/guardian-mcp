...
const eip712Verify = require('../src/eip712-verify');
...
module.exports = async function handler(req, res) {
  setCors(res);
  // Treat HEAD as GET (link-preview crawlers and uptime checks use it); the body is small HTML/JSON.
  const method = (req.method === 'HEAD' ? 'GET' : (req.method || 'GET')).toUpperCase();
  ...
  if (method === 'GET' && apiPath === '/verify-eip712-domain') {
    const q = new URLSearchParams((req.url || '').split('?')[1] || '');
    const rawTok = q.get('token') || '0x779ded0c9e1022225f8e0630b35a9b54be713736';
    const cid = Number(q.get('chainId') || 196);
    if (!/^0x[0-9a-fA-F]{40}$/.test(rawTok) || !Number.isInteger(cid) || cid <= 0) return send(res, 400, { ok: false, error: 'token (0x address) and chainId (positive integer) required' });
    try {
      const vr = await eip712Verify.verify(cid, rawTok, null, module.exports.eip712Fetch);
      res.setHeader('cache-control', 'public, max-age=600');
      return send(res, 200, Object.assign({ ok: true, note: 'Live eth_call of DOMAIN_SEPARATOR() (selector 0x3644e515). Only the accepted pair verifies on-chain; anything else a seller declares fails at settlement.' }, vr));
    } catch (e) { return send(res, 502, { ok: false, error: 'on-chain read failed: ' + e.message }); }
  }
  if (method === 'GET' && apiPath === '/demo') {
    res.statusCode = 200; res.setHeader('content-type', 'text/html; charset=utf-8'); res.setHeader('cache-control', 'public, max-age=300');
    return res.end('<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>GuardianMCP Pay-Safe — demo</title><style>html,body{margin:0;background:#0b0d10;color:#e6e9ee;font:15px/1.6 system-ui,Segoe UI,Roboto,sans-serif}.w{max-width:960px;margin:0 auto;padding:24px 16px 56px}h1{font-size:22px;margin:0 0 4px}p{color:#8d96a3;margin:6px 0}video{width:100%;border:1px solid #232a33;border-radius:12px;background:#000;margin-top:12px}a{color:#6aa9ff;text-decoration:none}</style></head><body><div class="w"><h1>GuardianMCP Pay-Safe — demo</h1><p><b>34 seconds:</b> a real headless AI agent quotes a seller on OKX.AI, Guardian returns DENY, the agent tries to pay anyway — and the hook blocks it. Unedited.</p><video controls preload="metadata" src="/demo-short.webm"></video><p style="margin-top:28px"><b>The full walkthrough (3:56)</b> — the same block at 0:00, then the whole flow.</p><video id="v" controls preload="metadata" src="/demo-v61.webm"><track kind="subtitles" srclang="en" label="English" src="/demo-v61.en.vtt" default></video><p><b>Chapters:</b> <a href="#" data-t="0">0:00 a real agent blocked</a> · <a href="#" data-t="103">1:43 MCP + sid 39876 DENY</a> · <a href="#" data-t="168">2:48 live agent, unedited</a> · <a href="#" data-t="192">3:12 real settled payment</a></p><script>document.querySelectorAll("a[data-t]").forEach(function(a){a.onclick=function(e){e.preventDefault();var v=document.getElementById("v");v.currentTime=+a.getAttribute("data-t");v.play()}})</script><p>Also: <a href="/agent-runs">/agent-runs</a> (the unedited runs) · <a href="/trust">/trust</a> · <a href="/company">/company</a> · <a href="/pay-safe">/pay-safe</a> · <a href="/demo-v61.en.srt">subtitles (.srt)</a> · <a href="https://youtu.be/hsvqdNWk5C4">watch on YouTube</a></p></div></body></html>');
  }
  ...
  if (method === 'GET' && (apiPath === '/stats' || apiPath === '/admin/stats')) {
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      try {
        ...
        if (!internal) { snap.external_users = 0; snap.paid_calls = 0; snap.note = 'Self-test and benchmark traffic plus a ScamSniffer-seeded registry; 0 external users and 0 paid calls so far. Not an adoption metric.'; }
