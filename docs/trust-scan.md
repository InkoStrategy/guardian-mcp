...
Generated 2026-09-25T08:01:33.128Z. One unpaid request per endpoint to capture the x402 challenge; Pay-Safe /check-payment against the listing. No payments, no signatures.
...
- **WARN endpoint_domain_suspicious** in Token Security Check (sid 40902): okx-asp.arkus.site matches a phishing pattern (brand_impersonation): Hostname contains the brand "okx" but is not an official okx domain. It is the endpoint named in the listing.
- **WARN eip712_domain_mismatch** in Token Security Check (sid 40902): extra declares the EIP-712 domain name "USD₂₀" version "1", but the USDT0 contract on xlayer uses name "USD₮0" version "1" (its DOMAIN_SEPARATOR 0xd591d9ba… matches only that pair). A TransferWithAuthorization signed with the declared domain will not verify on-chain.
...
| WARN | Token Security Check (sid 40902) | 0.02 USDT | 0.02 USDT0 on eip155:196 | endpoint_domain_suspicious, eip712_domain_mismatch |
...
| ALLOW | Asset Identifier (sid 40673) | 0.15 USDT | 0.15 USDT0 on eip155:196 |  |
...
| unreachable | TxPreflight Onchain Tx Guard (sid 40832) | 0.04 USDT | — | timeout |
