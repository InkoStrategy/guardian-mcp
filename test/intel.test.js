...
test('summary + safe_alternative: unlimited USDC approve to Uniswap router is explained and bounded', async () => {
  ...
  const r = await analyze({ to: USDC, data, context: { expected_amount: '150.5' } }, { reader: fakeChain() });
...
test('unknown_spender: unlimited approve to an unrecognised contract, proxy and tiny contracts are called out', async () => {
  const chain = fakeChain();
  const a = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [UNKNOWN_CONTRACT, MaxUint256]) }, { reader: chain });
  assert.deepEqual(new Set(a.reasons), new Set(['unlimited_approval', 'unknown_spender']));
  assert.equal(a.details.risk_score, 30);

  const p = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [PROXY_CONTRACT, MaxUint256]) }, { reader: chain });
  ...
  const t = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [TINY_CONTRACT, MaxUint256]) }, { reader: chain });
  assert.match(t.details.findings.find((x) => x.code === 'unknown_spender').message, /20 bytes of code/);

  const bounded = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [UNKNOWN_CONTRACT, 1000n]) }, { reader: chain });
...
test('address_poisoning: recipient imitating a known address is DENIED; the real address passes', async () => {
  ...
  const chain = fakeChain();
  const bad = await analyze({ to: USDC, data: erc20.encodeFunctionData('transfer', [poison, 1n]), context: { known_addresses: [real] } }, { reader: chain });
  assert.equal(bad.verdict, 'DENY');
  assert.ok(bad.reasons.includes('address_poisoning'));
  const good = await analyze({ to: USDC, data: erc20.encodeFunctionData('transfer', [real, 1n]), context: { known_addresses: [real] } }, { reader: chain });
  assert.equal(good.reasons.includes('address_poisoning'), false);
  const nativePoison = await analyze({ to: poison, data: '0x', value: '1', context: { known_addresses: [real] } }, { reader: chain });
...
test('contract_lookalike: approving a Permit2 look-alike is DENIED without any context', async () => {
  const fake = '0x0000' + 'ab'.repeat(16) + '8ba3';
  const r = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [fake, 1000n]) }, { reader: fakeChain() });
...
test('nested multicall: an approve hidden inside multicall is analysed like a top-level approve', async () => {
  ...
  const r = await analyze({ to: USDC, data }, { reader: fakeChain() });
  assert.equal(r.verdict, 'DENY');
...
test('router helpers: sweepToken / exactInputSingle to a third party -> WARN, to self -> ALLOW', async () => {
  ...
  const bad = await analyze({ to: SWAPROUTER02, data: router.encodeFunctionData('multicall', [[sweepOut]]), from: USER }, { reader: chain });
  ...
  const good = await analyze({ to: SWAPROUTER02, data: router.encodeFunctionData('multicall', [[swapSelf]]), from: USER }, { reader: chain });
...
test('Universal Router: PERMIT2_PERMIT to a foreign spender + TRANSFER to attacker are DENIED; honest plan is ALLOWED', async () => {
  ...
  const r = await analyze({ to: UNIVERSAL, data: evil, from: USER }, { reader: fakeChain() });
  assert.equal(r.verdict, 'DENY');
  ...
  const ok = await analyze({ to: UNIVERSAL, data: honest, from: USER }, { reader: fakeChain() });
...
test('simulation: revert reason is decoded and surfaced; insufficient balance is detected', async () => {
  ...
  const rev = await analyze({ to: USDC, data, from: USER }, { reader: fakeChain({ revert: 'ERC20: transfer amount exceeds balance' }) });
  ...
  const low = await analyze({ to: USDC, data, from: USER }, { reader: fakeChain({ balances: { [USDC.toLowerCase()]: 5000000n } }) });
  ...
  const noFrom = await analyze({ to: USDC, data }, { reader: fakeChain({ revert: 'x' }) });
...
test('simulation_unavailable: RPC failure during a requested simulation degrades to WARN', async () => {
  ...
  const r = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [V2ROUTER, 1n]), from: USER }, { reader: chain });
  assert.equal(r.verdict, 'WARN');
...
test('signature: ERC-2612 permit unlimited + far deadline to a wallet -> DENY', async () => {
  ...
  const r = await analyzeSignature({ type: 'eip712', from: USER, typedData }, { reader: fakeChain(), now: 1_800_000_000 });
  assert.equal(r.verdict, 'DENY');
  assert.ok(r.reasons.includes('approval_to_eoa'));
  assert.ok(r.reasons.includes('unlimited_approval'));
...
test('signature: bounded Permit2 to Universal Router with short deadline -> ALLOW; spoofed Permit2 domain -> DENY', async () => {
  ...
  const ok = await analyzeSignature({ type: 'eip712', from: USER, typedData: base }, { reader: fakeChain(), now });
  ...
  const bad = await analyzeSignature({ type: 'eip712', from: USER, typedData: spoofed }, { reader: fakeChain(), now });
  assert.equal(bad.verdict, 'DENY');
  ...
  const u = await analyzeSignature({ type: 'eip712', from: USER, typedData: unlimited }, { reader: fakeChain(), now });
...
test('signature: Permit2 PermitTransferFrom is flagged as an immediate transfer authorisation', async () => {
  ...
  const r = await analyzeSignature({ type: 'eip712', from: USER, typedData }, { reader: fakeChain(), now });
  assert.equal(r.verdict, 'WARN');
...
test('signature: Seaport order giving NFTs away for nothing -> DENY; fair order -> ALLOW', async () => {
  ...
  const drain = await analyzeSignature({ type: 'eip712', from: USER, typedData: mk([]) }, { reader: fakeChain(), now: 1_800_000_000 });
  assert.equal(drain.verdict, 'DENY');
  assert.ok(drain.reasons.includes('seaport_zero_consideration'));
  const zeroToOfferer = await analyzeSignature({ type: 'eip712', from: USER, typedData: mk([{ itemType: 0, token: '0x0000000000000000000000000000000000000000', identifierOrCriteria: '0', startAmount: '0', endAmount: '0', recipient: USER }]) }, { reader: fakeChain(), now: 1_800_000_000 });
  assert.ok(zeroToOfferer.reasons.includes('seaport_zero_consideration'));
  const fair = await analyzeSignature({ type: 'eip712', from: USER, typedData: mk([{ itemType: 0, token: '0x0000000000000000000000000000000000000000', identifierOrCriteria: '0', startAmount: '1000000000000000000', endAmount: '1000000000000000000', recipient: USER }]) }, { reader: fakeChain(), now: 1_800_000_000 });
...
test('signature: validation errors and context integration', async () => {
  ...
  const r = await analyzeSignature({ type: 'eip712', from: USER, typedData, context: { agent_goal: 'read' } }, { reader: fakeChain(), now: 1_800_000_000 });
  assert.equal(r.verdict, 'DENY');
...
test('backward compatibility: legacy shape unchanged, new fields present, fake readers without eth_call still work', async () => {
  ...
  const r = await analyze({ to: USDC, data: erc20.encodeFunctionData('approve', [V2ROUTER, 1n]) }, { reader: legacyReader });
