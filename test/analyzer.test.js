...
test('unlimited_approval: approve(MAX_UINT256) to a contract -> WARN', async () => {
  ...
  const r = await analyze({ to: TOKEN, data }, { reader, store: createMemoryStore() });
...
test('set_approval_for_all(true) -> DENY; revocation (false) -> ALLOW', async () => {
  ...
  const g = await analyze({ to: TOKEN, data: grant }, { reader, store: createMemoryStore() });
  const v = await analyze({ to: TOKEN, data: revoke }, { reader, store: createMemoryStore() });
...
test('fresh_recipient: native transfer (empty data, value > 0) -> WARN', async () => {
  const r = await analyze({ to: EOA_FRESH, data: '0x', value: '1000000000000000' }, { reader, store: createMemoryStore() });
  assert.equal(r.verdict, 'WARN');
  assert.deepEqual(r.reasons, ['fresh_recipient']);
...
test('zero_address: transfer to 0x0 -> DENY without touching the chain', async () => {
  ...
  const native = await analyze({ to: ZERO, data: '0x', value: '1' }, { store: createMemoryStore(), reader: fakeReader({}, { fail: true }) });
  assert.equal(native.verdict, 'DENY');
});

test('unknown_selector -> WARN', async () => {
  const r = await analyze({ to: ROUTER, data: '0xdeadbeef' + '00'.repeat(32) }, { reader, store: createMemoryStore() });
  assert.equal(r.verdict, 'WARN');
  assert.ok(r.reasons.includes('unknown_selector'));
});

test('known DEX selector to a contract -> ALLOW', async () => {
  const r = await analyze({ to: ROUTER, data: '0x7ff36ab5' + '00'.repeat(32) }, { reader, store: createMemoryStore() });
  assert.equal(r.verdict, 'ALLOW');
...
test('calldata_to_eoa: calling a function on a wallet address -> WARN', async () => {
  ...
  const r = await analyze({ to: EOA_ACTIVE, data }, { reader, store: createMemoryStore() });
  assert.equal(r.verdict, 'WARN');
...
test('FAIL-SAFE: RPC outage never yields ALLOW', async () => {
  const data = erc20.encodeFunctionData('approve', [ROUTER, 1n]);
  const r = await analyze({ to: TOKEN, data }, { store: createMemoryStore(), reader: fakeReader({}, { fail: true }) });
  assert.equal(r.verdict, 'WARN');
