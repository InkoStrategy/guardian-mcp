...
## Тарификация (пока выключена)

`PRICING_MODE=free` по умолчанию: ничего не тарифицируется. Модель на будущее: базовый `/analyze` бесплатен
навсегда, потому что каждый бесплатный вызов кормит общий реестр. Премиум‑слои (сессия, алерты, эталон, анализ
подписей) будут отдельным платным сервисом в OKX.AI с отдельной регистрацией и ревью. Режим `auto` включает
